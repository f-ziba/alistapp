...
def index(request):
    return HttpResponse("Order App installed")
...
def placeorder(request):
    api_key = 'sk_test_86d64c2518c8fdbfc149ef1f843e0a84b10e7ffa'
    url = 'https://api.paystack.co/transaction/initialize'
    callback_url = 'http://18.118.113.241/order/ordercompleted/'
...
def ordercompleted(request):
    ...
    offer = Product.object.get(offer=True)
    profile = UserProfile.objects.get(user_username = request.user.username)
    shopcart = ShopCart.objects.filter(order_placed=False).filter(user__username = request.user.username)
